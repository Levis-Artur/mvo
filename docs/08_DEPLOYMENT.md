...
## Обов’язкова 2FA і ключ TOTP

Production API вимагає `TOTP_ENCRYPTION_KEY` для шифрування TOTP-secret користувачів через AES-256-GCM. Ключ має бути canonical base64-представленням рівно 32 bytes. Створити його можна один раз командою:

```bash
openssl rand -base64 32
```

Значення зберігається тільки в production `.env`, не комітиться в Git і має мати окрему захищену резервну копію. Втрата ключа після налаштування 2FA зробить наявні TOTP-secret непридатними для розшифрування. Не змінюйте ключ під час звичайного deployment або restore без окремої процедури ротації.

Migration `20260825000500_revoke_legacy_sessions_for_mandatory_2fa` одноразово відкликає сесії, створені до впровадження обов’язкового другого фактора. Після rollout усі користувачі повинні пройти password pre-auth flow і налаштувати або підтвердити 2FA перед створенням нової authenticated session.
