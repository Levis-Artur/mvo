...
- Прямі залишки МВО як єдине джерело актуального стану
- Передачі `MVO_TRANSFER`, що зменшують тільки відправника, та видачі `ISSUE`
  з приватними вкладеннями накладних
- Бухгалтерський реєстр/CSV передач без автоматичного зіставлення з імпортами
...
- `ImportBatch`
- `ImportRow`
- `CustodyBalance` (read-only legacy archive)
- `StockDocument`, `StockDocumentLine`, `StockDocumentAttachment`
...
У колонці `Контрагент` очікується формат `<бухгалтерське ім'я>_<код МВО>`.
Код береться після останнього символу `_`, зберігається як string із початковими
нулями та має містити рівно чотири цифри. Автоматичне зіставлення виконується
лише за точним `externalAccountingCode`; ПІБ, внутрішній номер МВО та обліковий
запис не використовуються як fallback.

Рядки без коду, з некоректним кодом, невідомим або неактивним МВО отримують
статус `ERROR`. Якщо один код у межах файла вказаний для кількох різних
нормалізованих імен, усі рядки з цим кодом також блокуються як колізія.
...
Проведений імпорт збільшує `StockBalance` відповідного МВО. Передача в UI
створює `MVO_TRANSFER`: вона зменшує тільки залишок відправника й не створює
прихід одержувачу. Наступний CSV є незалежним офіційним приходом і автоматично
не зіставляється з передачею. Новий `ISSUE` зменшує прямий залишок; скасування
відновлює його reversal-транзакцією без видалення історії. Legacy `TRANSFER`,
`ASSIGNMENT` і `CustodyBalance` залишаються доступними лише для аудиту.
