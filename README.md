...
- Розділення прямого залишку, майна, закріпленого за іншими, та майна,
  закріпленого за поточним МВО
- Передачі `ASSIGNMENT` без зміни облікового власника та видачі `ISSUE` з
  приватними вкладеннями накладних
...
Проведений імпорт збільшує `DIRECT` баланс відповідного МВО. Передача в UI
створює `ASSIGNMENT`: вона переносить кількість у custody, не змінює accounting
owner і не збільшує `StockBalance` одержувача. Legacy `TRANSFER` доступний лише
для перегляду. `ISSUE` остаточно зменшує direct або custody bucket; скасування
створює reversal без видалення історії.
