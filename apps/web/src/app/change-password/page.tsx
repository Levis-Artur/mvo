'use client';

import { FormEvent, useEffect, useState } from 'react';
import { useRouter } from 'next/navigation';
import { ApiError } from '@/lib/api-client';
import { getDefaultAppPath, useAuth } from '../ui/auth-context';
import { AuthLoadingState } from '../ui/protected-mvo-app';

const MIN_PASSWORD_LENGTH = 12;

export default function ChangePasswordPage() {
  const router = useRouter();
  const { changePassword, loading, user } = useAuth();
  const [oldPassword, setOldPassword] = useState('');
  const [newPassword, setNewPassword] = useState('');
  const [confirmPassword, setConfirmPassword] = useState('');
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState('');

  useEffect(() => {
    if (!loading && !user) {
      router.replace('/login');
    }
  }, [loading, router, user]);

  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setError('');

    if (!oldPassword || !newPassword || !confirmPassword) {
      setError('Заповніть усі поля.');
      return;
    }

    if (newPassword.length < MIN_PASSWORD_LENGTH) {
      setError('Новий пароль має містити щонайменше 12 символів.');
      return;
    }

    if (newPassword !== confirmPassword) {
      setError('Підтвердження пароля не збігається.');
      return;
    }

    setSubmitting(true);
    try {
      const currentUser = await changePassword({ oldPassword, newPassword });
      router.replace(
        currentUser.mustChangePassword
          ? '/change-password'
          : getDefaultAppPath(currentUser),
      );
    } catch (requestError) {
      setError(
        requestError instanceof ApiError
          ? requestError.message
          : 'Не вдалося змінити пароль.',
      );
    } finally {
      setSubmitting(false);
    }
  }

  if (loading || !user) {
    return <AuthLoadingState />;
  }

  return (
    <main className="grid min-h-screen place-items-center bg-[var(--app-background)] px-4 py-8 text-[var(--text-primary)]">
      <section className="app-card w-full max-w-[460px]">
        <div className="border-b border-[var(--border)] bg-[var(--toolbar-background)] px-4 py-3">
          <p className="text-xs font-semibold text-[var(--primary)]">
            {user.username}
          </p>
          <h1 className="mt-1 text-lg font-semibold">Зміна пароля</h1>
          <p className="mt-1 text-xs text-[var(--text-secondary)]">
            Вкажіть поточний пароль і створіть новий пароль.
          </p>
        </div>

        <form className="grid gap-4 p-4" onSubmit={handleSubmit}>
          <label className="grid gap-1.5">
            <span className="text-xs font-medium text-[var(--text-secondary)]">
              Поточний пароль
            </span>
            <input
              autoComplete="current-password"
              className="input"
              disabled={submitting}
              type="password"
              value={oldPassword}
              onChange={(event) => setOldPassword(event.target.value)}
            />
          </label>

          <label className="grid gap-1.5">
            <span className="text-xs font-medium text-[var(--text-secondary)]">
              Новий пароль
            </span>
            <input
              autoComplete="new-password"
              className="input"
              disabled={submitting}
              minLength={MIN_PASSWORD_LENGTH}
              type="password"
              value={newPassword}
              onChange={(event) => setNewPassword(event.target.value)}
            />
          </label>

          <label className="grid gap-1.5">
            <span className="text-xs font-medium text-[var(--text-secondary)]">
              Підтвердження нового пароля
            </span>
            <input
              autoComplete="new-password"
              className="input"
              disabled={submitting}
              minLength={MIN_PASSWORD_LENGTH}
              type="password"
              value={confirmPassword}
              onChange={(event) => setConfirmPassword(event.target.value)}
            />
          </label>

          {error ? (
            <div className="border border-red-200 bg-red-50 px-3 py-2 text-sm text-[var(--danger)]">
              {error}
            </div>
          ) : null}

          <button className="btn btn-primary" disabled={submitting} type="submit">
            {submitting ? 'Збереження...' : 'Змінити пароль'}
          </button>
        </form>
      </section>
    </main>
  );
}
