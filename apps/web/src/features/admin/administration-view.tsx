...
export function AdministrationView() {
  ...
  return <section className="grid gap-4">
    ...
    <div className="administration-grid">
      ...
      <Card title="Режим небезпечних дій">{mode === 'checking' ? <LoadingState label="Перевірка стану на сервері…" /> : <div className="grid gap-3"><StatusBadge tone={mode === 'enabled' ? 'danger' : 'neutral'}>{mode === 'enabled' ? 'Увімкнений на сервері' : 'Вимкнений на сервері'}</StatusBadge>{mode === 'disabled' ? <p className="text-sm text-[var(--color-text-secondary)]">{DESTRUCTIVE_MODE_DISABLED_MESSAGE}.</p> : <p className="text-sm text-[var(--color-danger)]">Небезпечні дії доступні лише користувачу з роллю «Власник» та додатково перевіряються сервером.</p>}</div>}</Card>
      ...
      <Card title="Імпорти"><div className="grid gap-3"><p className="text-sm text-[var(--color-text-secondary)]">Контроль імпортів, відкочування та попередній перегляд видалення.</p><Button variant="outline" type="button" onClick={() => router.push('/imports')}>Відкрити імпорти</Button></div></Card>
      <Card title="Тестові дані"><div className="grid gap-3"><p className="text-sm text-[var(--color-text-secondary)]">Повне очищення тестових даних зі збереженням поточного користувача з роллю «Власник», аудиту та системних налаштувань.</p><Button disabled={mode !== 'enabled'} variant="danger" type="button" onClick={() => setResetOpen(true)}>Очистити тестові дані</Button>{mode === 'disabled' ? <small>{DESTRUCTIVE_MODE_DISABLED_MESSAGE}.</small> : null}</div></Card>
      <Card title="Аудит безпеки"><p className="text-sm text-[var(--color-text-secondary)]">Небезпечні дії записуються сервером у журнал аудиту. Окремий маршрут API для перегляду журналу безпеки зараз не надається.</p></Card>
    </div>
    {resetOpen ? <ResetTestDataModal onClose={() => setResetOpen(false)} onReset={() => setToast('Тестові дані очищено. Сервер створив запис аудиту.')} /> : null}
