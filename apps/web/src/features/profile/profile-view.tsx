...
export function ProfileView() {
  ...
  useEffect(() => {
    ...
    apiClient.responsiblePerson(user.responsiblePersonId).then(setPerson)
      .catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Не вдалося завантажити дані МВО.'))
  ...
  async function endAllSessions() {
    ...
    try { await logoutAll(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'Не вдалося завершити всі сесії.'); setSubmitting(false); }
  }

  return <main className="min-h-screen bg-[var(--color-workspace)]">
    <header className="app-topbar"><div className="app-topbar__inner"><div className="app-brand"><span className="app-brand__badge">МВО</span><strong className="app-brand__title">Облік майна МВО</strong></div><Button variant="ghost" type="button" onClick={() => void logout()}>Вийти</Button></div></header>
    <div className="page-container grid gap-4">
      ...
      <div className="profile-grid">
        ...
        <Card title="Пароль"><p className="mb-4 text-sm text-[var(--color-text-secondary)]">Зміна пароля виконується через захищений auth endpoint.</p><Button type="button" onClick={() => router.push('/change-password')}>Змінити пароль</Button></Card>
