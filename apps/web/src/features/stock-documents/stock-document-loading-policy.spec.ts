...
describe('stock document lazy loading policy', () => {
  ...
  it('loads transfer targets only when the MVO_TRANSFER form is opened', () => {
    ...
    expect(controller).toContain("if (nextType !== 'MVO_TRANSFER') return;");
  ...
  it('uses a compact MVO list without technical status labels', () => {
    ...
    expect(table).toContain("if (user.role === 'MVO' || user.role === 'ORG_MANAGER')");
    expect(table).toContain('documentDirectionPresentation(document).label');
  ...
  it('renders sequential numbers and a compact document table without exposing legacy identifiers', () => {
    ...
    const mvoStart = table.indexOf("if (user.role === 'MVO' || user.role === 'ORG_MANAGER')");
    ...
    expect(mvoTable).toContain("{ label: '№', className: 'stock-documents-table__number' }");
    expect(mvoTable).toContain("{ label: 'Кому / від кого', className: 'stock-documents-table__person' }");
    expect(mvoTable).toContain("{ label: 'Кількість', numeric: true }");
    expect(mvoTable).toContain("user.role === 'ORG_MANAGER' ? [{ label: 'Тип' }, { label: 'Відправник' }] : []");
    ...
    expect(mvoTable).toContain("{ label: 'Номенклатура' }");
    expect(mvoTable).toContain('documentNumberLabel(document.displayNumber)');
    expect(mvoTable).toContain('formatQuantity(document.totalQuantity)');
