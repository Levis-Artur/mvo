...
export function DeleteDocumentModal({ document, loading, error, onConfirm, onClose }: {
  ...
  return <Modal closeOnEscape={!loading} destructive footer={<><Button disabled={loading} variant="outline" type="button" onClick={onClose}>Закрити</Button><Button disabled={loading} variant="danger" type="button" onClick={onConfirm}>{loading ? 'Видалення…' : 'Видалити чернетку'}</Button></>} onClose={onClose} title="Видалення чернетки">
    <div className="grid gap-3"><p>Видалити чернетку документа <strong>{documentNumberLabel(document.displayNumber)}</strong>?</p><p className="text-sm text-[var(--color-text-secondary)]">Проведені та скасовані документи видаляти не можна.</p>{error ? <ErrorState message={error} /> : null}</div>
  </Modal>;
}
